"""
compute_bot_scores.py

Diagnostic script for exploring the in-degree/out-degree ratio
distribution, used to decide the threshold for flagging suspicious
accounts before applying it in the main pipeline (compute_bot_ids.py).

For each ego in personal_networks.pkl, computes:
  - out_degree: number of distinct alters contacted (already available
    via EgoInteractions.out_degree())
  - in_degree: number of distinct accounts that interacted WITH that
    ego (computed by inverting the structure, since EgoInteractions is
    organised by ego as source)
  - ratio = in_degree / out_degree

Saves a CSV with these values for every ego, a CDF plot of the ratio
(in figures/), and prints a percentile summary for a quick look
without opening the plot.

Usage:
    python3 compute_bot_scores.py <personal_networks.pkl> <output_csv> [output_png] [interaction_type]

Example (all types combined):
    python3 compute_bot_scores.py data/processed/personal_networks.pkl output/bot_scores.csv figures/bot_ratio_cdf.png

Example (a single interaction type):
    python3 compute_bot_scores.py data/processed/personal_networks.pkl output/bot_scores_vote.csv figures/bot_ratio_cdf_vote.png vote
"""

import sys
import os
import pickle
import csv
from collections import defaultdict

PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, PROJECT_ROOT)

from personalnetwork import INTERACTION_TYPES


def compute_degrees(personal_networks, interaction_type=None):
    """
    Computes out-degree and in-degree for every ego.

    interaction_type=None: combines all types (union of distinct
    alters across vote/comment/transfer).
    interaction_type='vote' (or another type): computes on that type
    only.
    """
    types_to_use = [interaction_type] if interaction_type else INTERACTION_TYPES

    out_degree = {}
    in_degree_sources = defaultdict(set)  # alter_id -> set of egos that contacted it

    for ego_id, ego in personal_networks.items():
        alters_reached = set()
        for alter_id, alter_data in ego.interactions.items():
            if any(alter_data.counts[t] > 0 for t in types_to_use):
                alters_reached.add(alter_id)
                in_degree_sources[alter_id].add(ego_id)
        out_degree[ego_id] = len(alters_reached)

    in_degree = {ego_id: len(in_degree_sources.get(ego_id, set())) for ego_id in personal_networks.keys()}

    return out_degree, in_degree


def compute_bot_scores(input_pickle, output_csv, output_png=None, interaction_type=None):
    with open(input_pickle, 'rb') as f:
        personal_networks = pickle.load(f)

    print(f"Computing in/out-degree for {len(personal_networks)} egos "
          f"({'all types combined' if interaction_type is None else interaction_type})...")

    out_degree, in_degree = compute_degrees(personal_networks, interaction_type)

    rows = []
    for ego_id in personal_networks.keys():
        od = out_degree[ego_id]
        idg = in_degree[ego_id]
        ratio = idg / od if od > 0 else 0.0
        rows.append((ego_id, od, idg, ratio))

    rows.sort(key=lambda r: r[3])

    with open(output_csv, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['ego_id', 'out_degree', 'in_degree', 'ratio'])
        writer.writerows(rows)
    print(f"Saved to {output_csv}")

    ratios = [r[3] for r in rows]
    print("\nIn-degree/out-degree ratio summary (percentiles):")
    import statistics
    n = len(ratios)
    for p in [1, 5, 10, 25, 50, 75, 90, 95, 99]:
        idx = min(int(n * p / 100), n - 1)
        print(f"  {p}th percentile: {ratios[idx]:.4f}")
    print(f"  Mean: {statistics.mean(ratios):.4f}")

    if output_png:
        try:
            import matplotlib
            matplotlib.use('Agg')
            import matplotlib.pyplot as plt
            import numpy as np

            cdf_y = np.arange(1, n + 1) / n
            plt.figure(figsize=(8, 5))
            plt.plot(ratios, cdf_y)
            # 'symlog' scale, not 'log': a pure log scale cannot
            # represent 0 (log(0) is undefined) and would silently
            # exclude it from the plot. An account with in_degree=0
            # (never reciprocated) has ratio exactly 0, the most
            # important case to see. 'symlog' handles zero correctly
            # (linear near 0, logarithmic elsewhere).
            plt.xscale('symlog', linthresh=0.01)
            plt.xlabel('in-degree / out-degree ratio')
            plt.ylabel('CDF')
            plt.title('In/out-degree ratio distribution'
                       + ('' if interaction_type is None else f' ({interaction_type})'))
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig(output_png, dpi=150)
            print(f"\nCDF plot saved to {output_png}")
        except ImportError:
            print("\nmatplotlib not available -- plot not generated, but the CSV is ready for analysis.")


if __name__ == '__main__':
    if len(sys.argv) not in (3, 4, 5):
        print(__doc__)
        sys.exit(1)
    output_png = sys.argv[3] if len(sys.argv) >= 4 else None
    interaction_type = sys.argv[4] if len(sys.argv) == 5 else None
    compute_bot_scores(sys.argv[1], sys.argv[2], output_png, interaction_type)
